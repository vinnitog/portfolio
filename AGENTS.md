...
Leia `PROJECT_CONTEXT.md` e `SKILLS.md` antes de alterar codigo, testes, docs ou configuracoes.
...
1. Descoberta de produto, quando o pedido envolver ideia, modelo de negocio, monetizacao ou preco
   - Produto novo: use `brainstorm-ideas-new`; produto existente: `brainstorm-ideas-existing`.
   - Use `business-model` para estruturar criacao, entrega e captura de valor.
   - Use `monetization-strategy` antes de escolher a fonte de receita e `pricing-strategy` para definir preco e experimento de validacao.

2. Privacidade antes do codigo, quando houver dados pessoais
   - Para uma feature nova que trata dados pessoais, use `lgpd-legal-basis` antes da implementacao.
   - Para auditoria ampla, legado ou duvida de cobertura, comece por `lgpd-audit`.
   - As skills apoiam conformidade, mas decisoes juridicas e publicacao de documentos exigem revisao humana especializada.
...
4. `impeccable`, quando houver front-end
   - Qualquer criacao, redesign, auditoria ou refinamento de interface deve usar a skill vendorizada `impeccable`.
   - Siga o workflow e os limites de verificacao definidos pela propria skill.

5. `code-reviewer`
   - Entra logo apos o `senior-dev`, ou logo apos o `impeccable` quando houver front-end.
...
Este repositorio e um no independente gerenciado pelo hub `togs-backoffice` com o id `portfolio`. Quando um caminho local for necessario, resolva-o pelo control plane ou pela variavel `TOGS_BACKOFFICE_PATH`; nao publique paths de usuario neste repositorio.
